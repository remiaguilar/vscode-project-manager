...
import { ProjectStorage, ProjectData } from './projectStorage';

export function activate(context: vscode.ExtensionContext) {
    console.log('Project Manager está activo');

    const projectStorage = new ProjectStorage(context);
    ...
    async function addExternalAppToFolder(targetFolder: string, appTypeValue: string) {
        ...
        if (appTypeValue === 'native') {
            ...
            let installedApps: Array<{label: string, name: string, bundleId: string}> = [];
            ...
            if (!appName) {
                ...
                try {
                    const { stdout } = await execAsync(
                        `defaults read "/Applications/${customName}.app/Contents/Info" CFBundleIdentifier 2>/dev/null`
                    );
        ...
        if (appUrl) { appData.appUrl = appUrl; }
        if (appBundleId) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('projectManager.selectDirectory', async () => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('projectManager.addProject', async () => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('projectManager.addCurrentProject', async () => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('projectManager.addProjectToFolder', async (folderItem: ProjectItem) => {
            if (!folderItem || !folderItem.isFolder) {return;}
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('projectManager.openProject', async (projectItem: ProjectItem) => {
            if (projectItem) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('projectManager.removeProject', async (projectItem: ProjectItem) => {
            if (projectItem) {
                const projects = projectStorage.loadProjects();
                ...
                if (projectItem.isExternalApp) {
                    ...
                    filtered = projects.filter(p => 
                        !(p.name === projectItem.label && p.isFolder)
                    );
                }
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('projectManager.openInFinder', async (projectItem: ProjectItem) => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('projectManager.createFolder', async () => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('projectManager.addExternalApp', async (preselectedType?: string) => {
            let appType: { label: string; value: string; description?: string } | undefined;
...
class ProjectManagerProvider implements vscode.TreeDataProvider<ProjectItem>, vscode.TreeDragAndDropController<ProjectItem> {
    ...
    dropMimeTypes = ['application/vnd.code.tree.projectManagerView'];
    dragMimeTypes = ['application/vnd.code.tree.projectManagerView'];
    ...
    public async handleDrag(source: ProjectItem[], dataTransfer: vscode.DataTransfer): Promise<void> {
    ...
    public async handleDrop(target: ProjectItem | undefined, dataTransfer: vscode.DataTransfer): Promise<void> {
        ...
        const draggedItems = transferItem.value as ProjectItem[];
    ...
    getChildren(element?: ProjectItem): Thenable<ProjectItem[]> {
        ...
        if (!element) {
            const rootItems = projects
                ...
                .map(project => {
                    if (project.isFolder) {
                        const hasActiveChild = projects.some(p => 
...
class ProjectItem extends vscode.TreeItem {
    constructor(
        ...
        public readonly isExternalApp: boolean = false,
